...
namespace BlueCardPortal.Core.Contracts
{
    ...
    {
        List<DocumentVM> CreateOtherDocument(Guid applicationId, DocumentsVM? appDocs, Guid? foreignerId);
        ...
        Task<GridResponseModel> GetComplaintList(string filterJson, int? inPage, long? inPageSize);
        ...
        Task<SeriesVM> GetPermitData(DashboardFilter filter);
        ApplicationUpdateVM InitNewApplicationUpdate(string applicationNumber);
        ComplaintVM InitNewComplaint(string applicationNumber);
        SelfDenialVM InitNewSelfDenial(string applicationNumber);
        Task RemoveDocument(Guid applicationId, Guid portalId, bool remove);
        ...
        Task SaveDocument(Guid applicationId, DocumentVM document);
        ...
        Task SetStatusNone(Guid applicationId);
